...
```{r inv_positions, results = 'asis'} 
  ...
  for (i in seq_along(inv_ORCID[[id]][[2]]$summaries)) {
    ...
    inv[i, 2] <- if_else(is.null(inv_ORCID[[id]][[2]]$summaries[[i]]$`invited-position-summary.end-date.year.value`),
                         "",
                         inv_ORCID[[id]][[2]]$summaries[[i]]$`invited-position-summary.end-date.year.value`)
  ...
  inv %>% 
    mutate(Year_End = str_sub(Year_End, -2), 
           Role = str_c("\\textbf{", Role, "}")) %>%
    unite(Year, Year_Start, Year_End, sep = "-", na.rm = TRUE) %>% 
    unite(Details, Role, Company, Department, City, Country, sep = ", ", na.rm = TRUE) %>% 
    CV_table()
...
```{r memberships, results = 'asis'}
  ...
  mem %>% 
    mutate(Company = str_c("\\textbf{", Company, "}")) %>%
    unite(Year, Year_Start, Year_End, sep = "-", na.rm = TRUE) %>% 
...
```{r awards, results = 'asis'}
  ...
  fund %>% 
    filter(Type == "award") %>%
    mutate(Title = str_c("\\textbf{", Title, "}")) %>%
...
```{r Articles, results = 'asis'}
  ...
  pubs %>%
    ...
    mutate(DOI = str_c("\\textcolor{blue}{", "http://dx.doi.org/", DOI, "}",  sep = "")) %>%
