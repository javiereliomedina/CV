...
# Function 1 for cleaning:  Education, employment, Invited Positions, service
format_ORCID_1 <- function(x) {
...
# Function 2 for cleaning: Memberships
format_ORCID_2 <- function(x) {
  x %>% 
    pluck("affiliation-group", "summaries") %>% 
    bind_rows() %>% 
    select(contains(c("start-date.year",
